...
export class UniversalapiService {
  // THIS IS HEROKU TEST DB
  // static url = 'https://connect-it-api.herokuapp.com';

  // static url = 'https://daily-grocery-api.herokuapp.com';

  // THIS IS CONNECT BAZAR MEERUT PROD DB
  static url = 'http://ec2-3-140-91-142.us-east-2.compute.amazonaws.com:8080';
  ...
  getAllSubCategories() {
    return this.http.get(UniversalapiService.url + '/subCategory/getAll');
  }

  getProducts(id) {
    return this.http
      .get<any>(UniversalapiService.url + '/product/getBySubCategory/' + id)
      .pipe(delay(200));
  }
  ...
  ratingForm(value, orderId) {
    ...
    return this.http
      .post<any>(UniversalapiService.url + '/order/feedback', param, {
        headers,
      })
      .pipe(map(res => res));
  }
  ...
  getFeedback(orderId) {
    ...
    headers = headers.set('token', localStorage.getItem('grocericatoken'));
    return this.http
      .get<any>(UniversalapiService.url + '/order/feedback/view/' + orderId, {
        headers,
      })
      .pipe(map(res => res));
  }
  ...
  emptyCart() {
    ...
    headers = headers.set('token', localStorage.getItem('grocericatoken'));
    return this.http
      .get<any>(UniversalapiService.url + '/cart/empty', { headers })
      .pipe(map(res => res));
  }
}
